...
public class BidService {
    ...
    public BidResponse createBid(Long userId, BidRequest request) {
        ...
        int maxBid = bidList.size()==0? auction.getStartPrice() : bidList.get(0).getPrice();
        if(request.getPrice()<=maxBid) throw new IllegalArgumentException("현재 최대 입찰가격보다 낮거나 동일합니다.");

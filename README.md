...
프로젝트의 주요 기능 및 성능 개선 작업을 정리한 문서입니다.
...
### 구현 기능
- 경매 시작 및 마감 시간을 기준으로 상태를 자동 변경.

### 주요 로직
- Lambda를 활용해 경매 상태를 실시간으로 자동화.

### 의사결정
- AWS EventBridge Scheduler를 채택:
  - 예약된 작업의 정확한 트리거 제공.
  - 실패 시 재시도 메커니즘 내장.
  - `@Scheduled`의 처리 지연 문제를 해결.

### 성능 개선
- 변경 전: 단일 스레드 처리, 평균 15초 소요.
- 변경 후: 멀티스레드 병렬 처리, 평균 5~10초로 단축.
- ThreadPool을 사용하여 최대 100개의 스레드로 병렬 실행.
...
### 구현 기능
- Redis 기반의 Redisson 분산 락을 활용하여 동시 입찰 문제 해결.

### 주요 로직
- Hash 구조를 사용하여 입찰 데이터를 관리.
- Key 값을 기준으로 최신 데이터를 비교해 최고 입찰가 확인.
- 필요한 부분에만 락을 적용하여 효율 극대화.
...
### 회고
- Redis 기반 동시성 제어가 효율적이었으나 TTL 관리가 다소 복잡.
- 락 적용 범위를 AOP 기반으로 조정해 개선 가능.
...
### 구현 기능
- SQS와 Lambda를 활용한 비동기 처리 기반 티켓 구매 대기열 시스템 구현.

### 주요 로직
- SQS를 사용해 대규모 요청 분산 처리.
- Lambda를 통해 대기열 데이터를 효율적으로 처리.
- RDS Proxy로 DB 연결 효율화를 통한 대량 트래픽 최적화.

### 의사결정
- Lambda와 SQS 배치 사이즈 최적화:
  - 대규모 트래픽 처리 문제 해결.

### 회고
- Lambda의 자동 확장성과 AWS 통합으로 처리 효율 극대화.
- Redis 중심 캐싱 구조를 활용하면 성능을 더욱 개선할 수 있음.
...
### 문제
- 대규모 데이터 처리 중 RDS에서 불필요한 연결 점유가 발생.
- 연결 제한 초과로 인해 데이터 처리 지연 문제 발생.

### 해결 방안
1. RDS Proxy를 활용:
   - 연결 재활용 및 최적화.
   - 테스트 시 Gradual Ramp-Up 방식 적용.
2. HikariCP 설정:
   - 최대/최소 연결 값을 조정하여 효율적 연결 관리.

### 결과
- RDS Proxy로 연결 풀 효율성 증가 및 안정성 확보.
- HikariCP를 통한 처리 속도 약 30% 향상, 오류 발생 빈도 0% 달성.
...
### 회고
- AWS의 다양한 서비스 활용으로 안정성과 확장성을 확보.
- Redis와 RDS Proxy를 결합해 성능 개선 효과 확인.

### 미래 계획
- Java 21 가상 스레드를 활용한 고성능 처리.
- AI 추천 시스템으로 맞춤형 입찰 전략 제공.
- 글로벌 사용자 대상 경매 이벤트 확대.
---

...
<details>
<summary>4. Redisson 분산락의 범위를 한정하여 입찰 속도 개선</summary>

### 주요 로직
입찰 최고가 캐시 조회 , RDS에 저장, 캐시에 새로운 입찰 금액 저장을 하나의 메서드로 추출

### 선택지
1. 함수형 분산 락 : 전역락 방식
2. 메서드 분리후 aop 형태 분산 락

### 의사결정
- 유저 100명이 입찰을 100개의 평균속도를 기준으로 속도 비교 결과 메서드 분리가 속도가 빨라서 선택
1. 함수형 분산 락 : 평균  2800ms->평균  1418ms
2. 메서드 분리 : 평균  2800ms->평균  610ms
